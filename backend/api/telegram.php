...
switch ($method) {
    case 'GET':
        // The broadcast log and schedule are admin-only — never expose them
        // publicly (titles, dates, statuses and internal error text).
    ...
    case 'POST':
        if ($action === 'send-now') {
            ...
            // Find the devotional
            // Resolve TODAY's devotional (admin timezone) once so both the
            // no-ID path and the ID-miss fallback use identical matching.
            $todayDev = null;
            {
            ...
            $dev = null;
            if ($devotionalId) {
                // The ID is a UUID generated by the API; trim any stray
                // whitespace and match the exact stored id.
                $devotionalId = trim((string)$devotionalId);
                $stmt = $pdo->prepare("SELECT * FROM devotionals WHERE id = ? LIMIT 1");
                $stmt->execute([$devotionalId]);
                $dev = $stmt->fetch();

                // Fall back to a case-insensitive title match only as a last
                // resort for legacy rows whose id drifted — never the wrong
                // day, so prefer today's date match over a title collision.
                if (!$dev) {
                    $stmt = $pdo->prepare("SELECT * FROM devotionals WHERE LOWER(title) = LOWER(?) LIMIT 1");
                    $stmt->execute([$devotionalId]);
                    $dev = $stmt->fetch();
                }
                if (!$dev) {
            ...
            requireSection('telegram-integration');
            // POST /api/telegram/reschedule { id, postTime, scheduledDate?, year? }
            $input = jsonInput();
            $rowId = trim((string)($input['id'] ?? ''));
            $postTime = trim((string)($input['postTime'] ?? ''));
            if ($rowId === '') {
                jsonError('id is required', 400);
            }
            if (!preg_match('/^([01]\d|2[0-3]):[0-5]\d$/', $postTime)) {
                jsonError('postTime must be in HH:MM 24h format', 400);
            }
            $scheduledDate = isset($input['scheduledDate']) && trim((string)$input['scheduledDate']) !== ''
                ? trim((string)$input['scheduledDate'])
                : null;
            $year = isset($input['year']) && (int)$input['year'] > 0 ? (int)$input['year'] : null;

            if (tgReschedule($rowId, $postTime, $scheduledDate, $year)) {
                logActivity('telegram_reschedule', "Broadcast re-scheduled for " . ($scheduledDate ?? 'its date') . " at {$postTime}.", 'telegram');
                jsonResponse(['success' => true, 'message' => 'Broadcast re-scheduled.']);
            }
 ...
 *
 * When $includeHeader is FALSE the Date + Title block is omitted so the
 * message starts at the Scripture section — used when the devotional photo
 * (whose caption already carries the date + title) was posted first, so the
 * date and title are never duplicated across the two messages.
 */
